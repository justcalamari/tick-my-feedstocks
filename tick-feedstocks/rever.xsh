...
def update_feedstock(gh, username, feedstock_name, hash, protocol='ssh',
                     hash_type='sha256', patterns=DEFAULT_PATTERNS):
    # first, let's grab the feedstock locally
    upstream = feedstock_url(feedstock_name, protocol=protocol)
    origin = fork_url(upstream, username)
    feedstock_reponame = feedstock_repo(feedstock_name)
